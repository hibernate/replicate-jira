...
/**
 * Note that for the objects in the payload we define only the very basic
 * properties. This is on purpose, since we "don't trust" the incoming data as
 * it will come from an "open" endpoint (cannot secure a webhook). Because of
 * that, we will just take the id of an object and then get the data we need by
 * calling REST API.
 */
