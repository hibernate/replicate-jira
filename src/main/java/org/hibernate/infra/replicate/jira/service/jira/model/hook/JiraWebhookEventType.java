...
public enum JiraWebhookEventType {
	...
	ISSUELINK_CREATED("issuelink_created") {
		...
		public Collection<Runnable> handlers(ReportingConfig reportingConfig, JiraWebHookEvent event,
			...
			// NOTE: for linking issues we will just trigger an "issue update" handling
			// instead of doing something special (at least for now):
	...
	ISSUELINK_DELETED("issuelink_deleted") {
		...
		public Collection<Runnable> handlers(ReportingConfig reportingConfig, JiraWebHookEvent event,
			...
			// NOTE: for linking issues we will just trigger an "issue update" handling
			// instead of doing something special (at least for now):
			return List.of(new JiraIssueLinkDeleteEventHandler(reportingConfig, context, event.issueLink.id,
					event.issueLink.sourceIssueId, event.issueLink.destinationIssueId,
					event.issueLink.issueLinkType.id));
		}
	},
	COMMENT_CREATED("comment_created") {
		...
		public Collection<Runnable> handlers(ReportingConfig reportingConfig, JiraWebHookEvent event,
			...
			if (event.issue == null || event.issue.id == null) {
				throw new IllegalStateException(
						"Trying to handle a comment event but issue id is null: %s".formatted(event));
			}
			return List
					.of(new JiraCommentUpsertEventHandler(reportingConfig, context, event.comment.id, event.issue.id));
		}
	},
