package org.hibernate.infra.sync.jira.mock;
...
public class SampleJiraRestClient implements JiraRestClient {
	...
	public JiraComments getComments(String issueKey, int startAt, int maxResults) {
		...
		comments.comments.add( sampleCopiedComment( Math.abs( Long.parseLong( issueKey.substring( issueKey.lastIndexOf( '-' ) ) ) ), 123L ) );
	...
	private JiraComment sampleComment(Long issueId, Long commentId) {
		try {
			return objectMapper.readValue( """
					...
					""".formatted( issueId, commentId ), JiraComment.class );
		}
		catch (JsonProcessingException e) {
			throw new RuntimeException( e );
		}
